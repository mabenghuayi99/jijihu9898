...
RESULT_FILE_PATH = "success_log2.txt"
SUMMARY_TXT_PATH = "success_summary2.txt"
...
def get_accounts():
    """直接读取本地 users2.txt 文件"""
    if os.path.exists("users2.txt"):
        with open("users2.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []

def get_passwords():
    """直接读取本地 passwords2.txt 文件"""
    if os.path.exists("passwords2.txt"):
        with open("passwords2.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []
...
def main():
    ...
    if not usernames or not passwords:
        print("[!] 错误：未找到有效的账号字典(users2.txt)或密码字典(passwords2.txt)！")
        return
