...
RESULT_FILE_PATH = "success_log4.txt"
SUMMARY_TXT_PATH = "success_summary4.txt"
...
def get_accounts():
    """直接读取本地 users4.txt 文件"""
    if os.path.exists("users4.txt"):
        with open("users4.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []

def get_passwords():
    """直接读取本地 passwords4.txt 文件"""
    if os.path.exists("passwords4.txt"):
        with open("passwords4.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []
...
def main():
    ...
    if not usernames or not passwords:
        print("[!] 错误：未找到有效的账号字典(users4.txt)或密码字典(passwords4.txt)！")
        return
