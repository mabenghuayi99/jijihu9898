...
RESULT_FILE_PATH = "success_log1.txt"
SUMMARY_TXT_PATH = "success_summary1.txt"
...
def get_accounts():
    """直接读取本地 users1.txt 文件"""
    if os.path.exists("users1.txt"):
        with open("users1.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []

def get_passwords():
    """直接读取本地 passwords1.txt 文件"""
    if os.path.exists("passwords1.txt"):
        with open("passwords1.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []
...
def main():
    ...
    if not usernames or not passwords:
        print("[!] 错误：未找到有效的账号字典(users1.txt)或密码字典(passwords1.txt)！")
        return
