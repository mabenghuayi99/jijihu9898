...
# ==================== Google Dorks 专属挖掘词（针对 SERPER 优化） ====================
KEYWORDS = [
    'inurl:"api/v1/client/subscribe"',
    'inurl:"osubscribe.php?sid="',
    'inurl:"sub?target=clash" OR inurl:"sub?target=v2ray"',
    'ext:txt "api/v1/client/subscribe?token="',
    'ext:txt "subscribe?token=" OR "sub?token="',
    'ext:yaml "type: hysteria2" OR "type: vless"',
    'site:pastebin.com "api/v1/client/subscribe"',
    'site:pastebin.com "subscribe?token=" OR "sub?token="',
    'site:telegra.ph "免费节点 订阅" OR "白嫖节点 链接"',
    'site:rentry.co "clash节点 分享" OR "机场 订阅地址"',
    'site:t.me/s/ "api/v1/client/subscribe?token="',
    'site:t.me/s/ "clash节点 分享" OR "v2ray节点 分享"',
    '"免费机场 订阅" "2026"',
    '"白嫖机场 订阅" clash',
    '"机场面板 订阅" v2board',
    '"机场 订阅链接" bit.ly OR tinyurl.com OR t.cn'
]
...
def main():
    ...
    print("🚀 真实订阅域名最终完整版 + SERPER Dorks 直链极速榨取启动")
    ...
    # ========== 1. 关键词搜索 (高级 Google Dorks) ==========
    print(f"\n{'='*60}")
    print(f"===== ① 关键词搜索（高级 Google Dorks 捕获公开泄露）=====")
    print(f"{'='*60}")
    for idx, kw in enumerate(KEYWORDS, 1):
        print(f"\n[{idx}/{len(KEYWORDS)}] 执行 Dork: {kw}")
    ...
    for idx, domain in enumerate(seeds, 1):
        ...
        # 升级版套路一：域名加双引号，且强制要求网页快照中必须包含 token/sid 等关键参数尾巴
        query = f'"{domain}" "token=" OR "sid=" OR "clash=" OR "sub="'
    ...
    # ========== 3. 深度提取（从网页/分享贴中扒出直链） ==========
    # Serper 搜回来的可能只是包含订阅的网页（如 Pastebin 贴文），需要深入网页把纯直链抠出来
    ...
    save_and_send(all_direct_links, full_filename,
                  f"📋 完整纯直链候选列表（Google Dorks 精准提取版）\n时间: {now}\n数量: {len(all_direct_links)}")
